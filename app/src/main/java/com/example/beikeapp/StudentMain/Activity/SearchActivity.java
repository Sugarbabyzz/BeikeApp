...
import com.example.beikeapp.StudentMain.View.MyListView;
...
public class SearchActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private boolean isLv = false;   //判断List是否已创建
    private int num = 0, limit = 10;   //限制搜索结果最大条数
    ...
    public void onClick(View v) {
        if (!sv.getText().toString().isEmpty()) {
            searchResult(sv.getText().toString());
        } else {
    ...
    private void searchResult(final String text) {
        ...
        if (num > 0) {
            if (!isLv) {//如果lv未创建
                adapter = new SimpleCursorAdapter(this, R.layout.student_search_list_item, cursor,
